...
async def submit_onboarding(req: OnboardingSubmitRequest, db: AsyncSession = Depends(get_db)):
    ...
    user.career_clarity = req.career_clarity or ""
    user.current_status = req.current_status or ""
...
async def get_profile(user_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "profile": {
            ...
            "target_industries": user.target_industries,
            "graduation_year": user.graduation_year,
            "career_clarity": user.career_clarity,
...
async def update_profile(req: ProfileUpdateRequest, db: AsyncSession = Depends(get_db)):
    ...
    user.updated_at = datetime.utcnow()
    await db.commit()
    return {"success": True, "nickname": user.nickname, "avatar_url": user.avatar_url}
